...
class FireBaseUtilsSearch {
  ...
  static Future<List<ItemData>> getAllMerchantItemsByName(String itemName) async {
    try {
      List<ItemData> allItems = await getAllMerchantItems();

      // Filter items by category
      List<ItemData> itemsWithName = allItems.where((item) => item.title!.contains(itemName)).toList();
print(itemsWithName[0].title);
      return itemsWithName;
    } catch (e) {
      print('Error fetching merchant items by name: $e');
      throw e; // Rethrow the exception to handle it outside if needed
    }
  }
}
